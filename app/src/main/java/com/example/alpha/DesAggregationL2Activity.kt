...
class DesAggregationL2Activity : AppCompatActivity() {
    // UI-элементы
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_des_aggregation_l2)
        Log.d("DesAggregationL2Activity", "onCreate: Активность создана")

        toolbar = findViewById(R.id.toolbar)
        setSupportActionBar(toolbar)
        supportActionBar?.title = "Разагрегация паллеты"
        ...
        barcodeManager.addListener { barcodeType, barcodeData ->
            runOnUiThread {
                Log.d("DesAggregationL2Activity", "Сканирован код: $barcodeType | $barcodeData")
        ...
        WebSocketManagerInstance.webSocketManager?.setMessageListener { message ->
            runOnUiThread {
                Log.d("DesAggregationL2Activity", "Получено сообщение от WebSocket: $message")
                handleWebSocketResponse(message)
            }
        }

        btnCancel.setOnClickListener {
            Log.d("DesAggregationL2Activity", "Нажата кнопка 'Отмена'")
            resetScreen()
        }

        btnDesagg.setOnClickListener {
            Log.d("DesAggregationL2Activity", "Нажата кнопка 'Разагрегировать'")
    ...
    override fun onResume() {
        ...
        Log.d("DesAggregationL2Activity", "onResume: Активность возобновлена")
        showInitialMessage()
    }
    ...
    override fun onPause() {
        ...
        Log.d("DesAggregationL2Activity", "onPause: Активность приостановлена")
    }

    private fun showInitialMessage() {
        infoContainer.removeAllViews()
        addTextView("Считайте код паллеты или вложения", Color.DKGRAY)
    }
    ...
    private fun sendCodeCheckForDesagg(typeCode: String, value: String) {
        ...
        if (isWebSocketConnected()) {
            ...
            Log.d("DesAggregationL2Activity", "Отправлено сообщение: $json")
        } else {
    ...
    private fun updateStatusIndicator(color: Int) {
        Log.d("AggregationL2PrintActivity", "Обновление индикатора статуса с цветом: $color")
        ...
        try {
            ...
            when (code) {
                "code_check_for_desagg" -> {
                    if (status == -1) {
                        Log.e("DesAggregationL2Activity", "Ошибка проверки кода: $message")
                        showError(message)
                    } else if (status == 0) {
                        Log.d("DesAggregationL2Activity", "Код успешно проверен")
                        displayBoxInfo(jsonObject)
                    }
                }
                "Desaggregation" -> {
                    if (status == -1) {
                        Log.e("DesAggregationL2Activity", "Ошибка разагрегации: $message")
                        showError(message)
                    } else if (status == 0) {
                        Log.d("DesAggregationL2Activity", "Разагрегация успешна: $message")
                        infoContainer.removeAllViews()
            ...
            Log.e("DesAggregationL2Activity", "Ошибка парсинга ответа", e)
        }
    }
    ...
    private fun displayBoxInfo(json: JSONObject) {
        ...
        if (values != null && values.length() > 0) {
            ...
            addDivider()
            addTextView("Код паллеты: ${box.optString("code_value")}")
            addDivider()
            ...
            addDivider()
            addTextView("Номер паллеты: ${box.optString("num")}")
            addDivider()
    ...
    private fun showDesaggConfirmationDialog() {
        AlertDialog.Builder(this)
            ...
            .setMessage("Вы уверены, что хотите разагрегировать эту паллету?")
    ...
    private fun sendDesaggRequest() {
        ...
        Log.d("DesAggregationL2Activity", "Отправлен запрос на разагрегацию: $json")
    }
    ...
    private fun sendMessageIfConnected(message: String) {
        if (isWebSocketConnected()) {
            ...
            Log.d("DesAggregationL2Activity", "Сообщение отправлено: $message")
        }
    }
    ...
    private fun resetScreen() {
        Log.d("DesAggregationL2Activity", "Сброс состояния экрана")
